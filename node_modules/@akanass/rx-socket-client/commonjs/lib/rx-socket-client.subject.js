...
class RxSocketClientSubject extends rxjs_1.Subject {
    ...
    onBytes$() {
        return this
            .pipe(operators_1.map((message) => (message.type && message.type === 'binary' && message.binaryData) ?
            message.binaryData :
            message));
    }
    /**
    ...
    _message$(event) {
        return this
            .pipe(operators_1.map((message) => (message.type && message.type === 'utf8' && message.utf8Data) ?
            message.utf8Data :
            message), operators_1.filter((message) => message.event &&
            message.event !== 'close' &&
            message.event === event &&
            message.data));
    }
